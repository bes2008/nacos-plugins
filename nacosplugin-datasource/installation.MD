...
```text
io.github.qhsword.nacosplugins:nacosplugin-all:jar:VERSION           
```
...
想要执行SQL脚本文件，需要提供的命令行工具：${dmdbms}/bin/disql
...
```properties
# GoldenDB
spring.sql.init.platform=goldendb
db.num=1
db.url.0=jdbc:goldendb://localhost:3306/nacos
db.user.0=root
db.password.0=root_password
...
4. jdbc driver选择
   + 在JDK8上运行时，选择：com.oracle.database.jdbc:ojdbc8:23.26.0.0.0
   + 在JDK11~JDK21上运行时，选择：com.oracle.database.jdbc:ojdbc11:23.26.0.0.0
   + 在JDK6~JDK8上运行时，选择：com.oracle.database.jdbc:ojdbc6:11.2.0.4
